...
class K_dimensional_Tree():
	...
	def createTree(self, depth, data_set):
		...
		data_set.sort(key=lambda x: x[depth_next])
		depth_pos = len(data_set) // 2 #找到中位数的位置
		median = data_set[depth_pos] #中位数分割点
		# 不像公式i = j(mod k) + 1 是因为,x向量是从0开始的,而书上的是从1开始的；
		return KNode(median, depth_next,
					 self.createTree(depth_next + 1, data_set[: depth_pos]),
					 self.createTree(depth_next + 1, data_set[depth_pos + 1 :]))
